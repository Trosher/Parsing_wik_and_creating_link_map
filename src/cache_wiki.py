...
class Graph:
    ...
    def check(self, str):
        return self.list_place[str] if str in self.list_place else False
    ...
    def load_graph(self, path):
        ...
        if res:
            self.data = nx.DiGraph(res) if self.flag else nx.Graph(res)
    ...
    for _ in range(args.d):
        ...
        for _ in range(args.d):
            for name in temp_1:
                ...
                logging.error("Failed to process links")
            if graph.data.number_of_nodes() > 1000:
                break
...
def main():
    ...
    graph = deep_counter(Graph(True), args)
